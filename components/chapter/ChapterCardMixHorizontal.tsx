...
interface ChapterCardMixHorizontalProps {
  config: {
    id: string;
    title: string;
    description: string;
...
export default function ChapterCardMixHorizontal({ config, transparencyLevel = 4 }: ChapterCardMixHorizontalProps) {
  ...
  return (
    ...
    >
      <div 
        className={`
          relative bg-gradient-to-r ${colors.bgGradient} rounded-xl shadow-md border border-white/50 p-5 h-32
      ...
      >
        ...
        {/* Overlay avec transparence légère (3-4) */}
        {backgroundImage && (
          <div className={`absolute inset-0 bg-gradient-to-r ${overlayClass} rounded-xl`} />
        )}
        
        {/* Layout "mix horizontal" - plus haut que horizontal classique */}
        <div className="relative z-10 flex items-center h-full gap-5">
          
          {/* Personnage à gauche */}
          {config.character && (
            <div className="flex-shrink-0">
              <div className={`w-20 h-20 rounded-full bg-gradient-to-br ${colors.characterBg} p-1 shadow-md relative`}>
                {!imageError ? (
                  <img
          ...
          {/* Contenu principal - Plus d'espace vertical */}
          <div className="flex-1 min-w-0 flex flex-col justify-between h-full py-1">
            
            {/* Section haute : Titre et infos */}
            <div>
