...
interface ChapterCardTransparencyTestProps {
  config: {
    id: string;
    title: string;
    description: string;
...
export default function ChapterCardTransparencyTest({ config, transparencyLevel }: ChapterCardTransparencyTestProps) {
  ...
  // 🎨 Niveaux de transparence (1-10)
  const getTransparencyOverlay = () => {
    const overlays = {
      1: 'from-white/95 via-white/90 to-white/85', // Très transparent (5-15% d'image visible)
      2: 'from-white/90 via-white/85 to-white/75', // Très léger (10-25% d'image visible)
      3: 'from-white/85 via-white/75 to-white/65', // Léger (15-35% d'image visible)
      4: 'from-white/80 via-white/70 to-white/55', // Doux (20-45% d'image visible)
      5: 'from-white/75 via-white/65 to-white/45', // Modéré (25-55% d'image visible)
      6: 'from-white/70 via-white/55 to-white/35', // Visible (30-65% d'image visible)
      7: 'from-white/65 via-white/45 to-white/25', // Bien visible (35-75% d'image visible)
      8: 'from-white/55 via-white/35 to-white/15', // Très visible (45-85% d'image visible)
      9: 'from-white/45 via-white/25 to-white/10', // Presque opaque (55-90% d'image visible)
      10: 'from-white/35 via-white/15 to-white/5'  // Maximum (65-95% d'image visible)
    };
    ...
    <div className="relative">
      ...
      >
        ...
        >
          ...
          {/* Overlay avec niveau de transparence variable */}
          {backgroundImage && (
            ...
            {config.character && (
              <div className="flex-shrink-0">
                <div className={`w-16 h-16 rounded-full bg-gradient-to-br ${colors.characterBg} p-1 shadow-md relative`}>
                  ...
                  {/* Badge de statut sur le personnage */}
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between mb-2">
                <div className="min-w-0 flex-1">
                  <h3 className={`font-bold ${colors.textColor} text-base leading-tight truncate`}>
                    {config.title}
                  </h3>
