...
interface ChapterCardPersonnageHorizontalProps {
  config: {
    id: string;
    title: string;
    description: string;
...
export default function ChapterCardPersonnageHorizontal({ config }: ChapterCardPersonnageHorizontalProps) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {backgroundImage && (
          <div className="absolute inset-0 bg-gradient-to-r from-white/85 via-white/70 to-white/40 rounded-xl" />
        )}
        ...
        {/* Effet de brillance */}
        <div className="absolute inset-0 bg-gradient-to-r from-white/0 via-white/10 to-white/0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-xl pointer-events-none" />
      </div>
    </Link>
  );
}
