...
export default function CinquiemePage() {
  const [hoveredChapter, setHoveredChapter] = useState<string | null>(null)
  
  const cinquiemeChapters = getChaptersByClass('5eme')
  const config = { color: '#ff9ff3', icon: '🧮', name: '5ème' }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-6">
              ...
              <div className="flex items-center space-x-3">
                <div className="w-12 h-12 bg-gradient-to-br from-pink-500 to-purple-500 rounded-2xl flex items-center justify-center text-white text-2xl font-bold shadow-lg">
                  {config.icon}
                </div>
                <div>
                  <h1 className="text-3xl font-bold text-gray-900">{config.name}</h1>
                  <p className="text-gray-600 mt-1">Découvrez les mathématiques de 5ème</p>
                </div>
              </div>
            </div>
          </div>
